...
- A prepared Qwen3 model in `app/runtime/models`.

## Install and inspect

From `app/packages/local-llm` in the final layout:
...
`app/packages/benchmark-runner/results/<run-id>/`, not in the API source tree. Record
...
3. `docker compose config -q` passes from `app/runtime/deployment`.
...
- `docker compose --profile benchmark-api --profile benchmark -f
  app/runtime/deployment/compose.yaml config -q` passes.

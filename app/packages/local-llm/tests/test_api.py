...
from local_llm.interfaces.http.schemas import HealthResponse
...
def test_explicit_model_is_accepted_and_default_is_preserved() -> None:
    provider = FakeProvider()
    with make_client(provider) as client:
        selected = client.post("/v1/generate/sync", json={"model": "other-model", "text": "Oi"})
...
def test_unknown_model_is_rejected_before_provider() -> None:
    provider = FakeProvider()
    with make_client(provider) as client:
        response = client.post("/v1/generate/sync", json={"model": "not-allowed", "text": "Oi"})
...
def test_benchmark_status_and_report_are_scoped_to_run() -> None:
    with make_client() as client:
        ...
        report = client.get("/v1/benchmarks/benchmark-20260907-120000-a1b2c3d4/report?format=html")

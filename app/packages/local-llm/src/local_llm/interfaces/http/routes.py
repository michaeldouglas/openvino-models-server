...
from local_llm.adapters.errors import InferenceError, ModelNotFoundError
        ...
        try:
            yield _openai_sse(
                ChatCompletionChunk(
                    ...
                    created=created,
                    model=model_name,
                    choices=[ChatCompletionChunkChoice(delta=ChatChunkDelta(role="assistant"))],
                ...
                if chunk.text:
                    yield _openai_sse(
                        ChatCompletionChunk(
                            ...
                            model=model_name,
                            choices=[
                                ChatCompletionChunkChoice(delta=ChatChunkDelta(content=chunk.text))
                            ],
                        ).model_dump(exclude_none=True)
                    )
                if chunk.done:

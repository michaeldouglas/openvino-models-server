...
`models/.ov_cache` em `/opt/cache` e desativa o polling de configuração durante
...
O executor fica em `services/benchmark`, com imagem, dependências, testes e
...
Os resultados são gravados em `services/benchmark/results/<run-id>/` e não são
